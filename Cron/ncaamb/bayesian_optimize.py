#!/usr/bin/env python3
"""
Bayesian Optimization for Over/Under model using Optuna

This script:
1. Loads features from multiple years
2. Uses Bayesian optimization (Optuna) to find best parameters
3. Intelligently samples parameter space based on past results
4. Much more efficient than random search (~50-100 trials vs 500+ random)
...
def main():
    print("="*80)
    print("BAYESIAN OPTIMIZATION - XGBOOST MODEL (Optuna)")
    print("="*80)
    ...
    print("   This intelligently explores the parameter space based on past results\n")
    ...
    # Save results
    print(f"\n4. Saving results...")
    trials_df.to_csv("bayesian_optimization_results.csv", index=False)
    print(f"   Saved all {len(trials_df)} trials to bayesian_optimization_results.csv")
    ...
    top_10 = trials_df.nsmallest(10, 'value')[['number', 'value', 'params_learning_rate', 'params_max_depth',
                                                   'params_n_estimators', 'params_min_child_weight',
                                                   'params_subsample', 'params_colsample_bytree']]

    print("\n" + " "*5 + f"{'Trial':<7} {'Test MAE':<12} {'LR':<10} {'Depth':<7} {'Est':<7} {'MCW':<6} {'SS':<7} {'CS':<7}")
    print("="*90)

    for idx, row in top_10.iterrows():
        print(f"   {int(row['number']):<7} {row['value']:<12.4f} {row['params_learning_rate']:<10.6f} {int(row['params_max_depth']):<7} {int(row['params_n_estimators']):<7} {int(row['params_min_child_weight']):<6} {row['params_subsample']:<7.3f} {row['params_colsample_bytree']:<7.3f}")
    ...
    print(f"Study name: xgboost_optimization")
