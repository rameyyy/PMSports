#!/usr/bin/env python3
"""
Moneyline XGBoost Model - Bayesian Optimization and Training
Loads all featuresYYYY.csv files, optimizes hyperparameters, trains final model
Saves to models/moneyline/saved/xgboost_model_final.pkl
"""
...
print("="*80)
print("MONEYLINE XGBOOST - BAYESIAN OPTIMIZATION (250 ITERATIONS)")
print("="*80 + "\n")
...
def load_all_features():
    ...
    # Look for features{year}.csv files
...
if __name__ == "__main__":
    ...
    final_acc = accuracy_score(y_all, final_model.predict(X_all))
    ...
    save_path = save_dir / "xgboost_model_final.pkl"

    with open(save_path, 'wb') as f:
        pickle.dump(final_model, f)

    print(f"✓ Saved XGBoost Moneyline model trained on features{years[0]}-{years[-1]}")
    print(f"  File: {save_path}")

    # Save hyperparameters
    params_path = save_dir / "xgboost_final_hyperparameters.txt"
    with open(params_path, 'w') as f:
        f.write("XGBoost Final Model Hyperparameters\n")
        f.write("="*50 + "\n\n")
