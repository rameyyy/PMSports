#!/usr/bin/env python3
"""
Moneyline LightGBM Model - Bayesian Optimization and Training
Loads all featuresYYYY.csv files, optimizes hyperparameters, trains final model
Saves to models/moneyline/saved/lightgbm_model_final.pkl
"""
...
print("="*80)
print("MONEYLINE LIGHTGBM - BAYESIAN OPTIMIZATION (250 ITERATIONS)")
print("="*80 + "\n")
...
def objective(**params):
    ...
    # Evaluate - LightGBM returns probabilities, threshold at 0.5
    ...
    # Minimize: negative test accuracy + overfitting penalty
    overfit_penalty = max(0, train_acc - test_acc - 0.02) * 2
...
if __name__ == "__main__":
    ...
    after = len(df)
    print(f"Removed {before - after} games without ML odds")
    ...
    print("STEP 6: Bayesian Optimization (250 iterations)")
    print("-"*80)
    iteration_count = 0
    ...
    y_all = df_with_year.select('ml_target').to_numpy().ravel()
    ...
    final_acc = accuracy_score(y_all, (final_model.predict(X_all) >= 0.5).astype(int))
    ...
    save_path = save_dir / "lightgbm_model_final.pkl"

    with open(save_path, 'wb') as f:
        pickle.dump(final_model, f)

    print(f"✓ Saved LightGBM Moneyline model trained on features{years[0]}-{years[-1]}")
    print(f"  File: {save_path}")

    # Save hyperparameters
    params_path = save_dir / "lightgbm_final_hyperparameters.txt"
    with open(params_path, 'w') as f:
        f.write("LightGBM Final Model Hyperparameters\n")
        f.write("="*50 + "\n\n")
