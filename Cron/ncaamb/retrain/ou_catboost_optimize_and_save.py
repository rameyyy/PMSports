#!/usr/bin/env python3
"""
Over/Under CatBoost Model - Bayesian Optimization and Training
Loads all featuresYYYY.csv files, optimizes hyperparameters, trains final model
Saves to models/overunder/saved/catboost_model.pkl
"""
...
print("="*80)
print("OVER/UNDER CATBOOST - BAYESIAN OPTIMIZATION (250 ITERATIONS)")
print("="*80 + "\n")
...
space = [
    ...
    Real(0.5, 1.0, name='random_strength'),
    Real(0.5, 1.0, name='bagging_temperature'),
...
if __name__ == "__main__":
    ...
    final_model.fit(X_all, y_all, sample_weight=weights_all)
    ...
    save_path = save_dir / "catboost_model.pkl"

    final_model.save_model(str(save_path), format='cbm')
    print(f"✓ Saved CatBoost model trained on features{years[0]}-{years[-1]}")
    print(f"  File: {save_path}")

    # Save hyperparameters
    params_path = save_dir / "catboost_hyperparameters.txt"
    with open(params_path, 'w') as f:
        f.write("CatBoost O/U Model Hyperparameters\n")
        f.write("="*50 + "\n\n")
