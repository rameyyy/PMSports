#!/usr/bin/env python3
"""
Bayesian Optimization for LightGBM Over/Under model using Optuna

This script:
1. Loads features from multiple years
2. Uses Bayesian optimization (Optuna) to find best LightGBM parameters
3. Intelligently samples parameter space based on past results
4. Much more efficient than random search
5. Saves best parameters and trial history
"""
import polars as pl
import os
import sys
...
def load_features():
    ...
    if not dfs:
        raise FileNotFoundError("No feature files found (features.csv, features2021.csv, features2022.csv, features2023.csv)")
...
def main():
    print("="*80)
    print("BAYESIAN OPTIMIZATION - LIGHTGBM MODEL (Optuna)")
    print("="*80)
    ...
    print("   Testing 300 trials with LightGBM\n")
    ...
    print("\n" + "="*80)
    print("OPTIMIZATION COMPLETE - LIGHTGBM")
    print("="*80)
    ...
    # Save results
    print(f"\n4. Saving results...")
    trials_df.to_csv("bayesian_optimization_lgb_results.csv", index=False)
    print(f"   Saved all {len(trials_df)} trials to bayesian_optimization_lgb_results.csv")

    # Show top 10 trials
    print(f"\n" + "="*80)
    print(f"TOP 10 TRIALS - LIGHTGBM")
    print(f"="*80)
    top_10 = trials_df.nsmallest(10, 'value')[['number', 'value', 'params_learning_rate', 'params_max_depth',
                                                   'params_num_leaves', 'params_min_child_samples',
                                                   'params_subsample', 'params_colsample_bytree']]

    print("\n" + " "*5 + f"{'Trial':<7} {'Test MAE':<12} {'LR':<10} {'Depth':<7} {'Leaves':<7} {'MCS':<6} {'SS':<7} {'CS':<7}")
    print("="*90)

    for idx, row in top_10.iterrows():
        print(f"   {int(row['number']):<7} {row['value']:<12.4f} {row['params_learning_rate']:<10.6f} {int(row['params_max_depth']):<7} {int(row['params_num_leaves']):<7} {int(row['params_min_child_samples']):<6} {row['params_subsample']:<7.3f} {row['params_colsample_bytree']:<7.3f}")

    # Summary statistics
    print(f"\n" + "="*80)
    print(f"OPTIMIZATION STATISTICS - LIGHTGBM")
    print(f"="*80)
    ...
    print(f"\nResults saved to: bayesian_optimization_lgb_results.csv")
    print(f"Study name: lightgbm_optimization")
