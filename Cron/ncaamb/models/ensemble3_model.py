...
class Ensemble3Model:
    ...
    """
    def __init__(self, xgb_weight: float = 0.33, lgb_weight: float = 0.33, cat_weight: float = 0.34) -> None:
        self.xgb_model: Optional[OUModel] = None
        self.lgb_model: Optional[LGBModel] = None
        self.cat_model: Optional[CatModel] = None
        ...
        self.cat_weight = cat_weight / total_weight
    ...
    def train(
        ...
        cat_metrics = self.cat_model.train(features_df, test_size=test_size, **cat_params)
        ...
        ensemble_test_mae = (
            ...
            self.cat_weight * cat_metrics["test_mae"]
        )
        ...
        return {
            ...
            "cat_train_mae": cat_metrics["train_mae"],
            "cat_test_mae": cat_metrics["test_mae"],
    ...
    def predict(self, features_df: pl.DataFrame) -> Dict:
        """
        Make ensemble predictions by averaging all three model predictions.
        """
        if not self.is_trained or self.xgb_model is None or self.lgb_model is None or self.cat_model is None:
        ...
        cat_pred = self.cat_model.predict(features_df)

        # Ensemble predictions (weighted average)
        xgb_pred_array = np.array(xgb_pred["predicted_total"])
        lgb_pred_array = np.array(lgb_pred["predicted_total"])
        cat_pred_array = np.array(cat_pred["predicted_total"])

        ensemble_pred = (
            self.xgb_weight * xgb_pred_array +
            self.lgb_weight * lgb_pred_array +
            self.cat_weight * cat_pred_array
        )
        ...
        return {
            ...
            "cat_predicted_total": cat_pred["predicted_total"],
    ...
    def get_feature_importance_comparison(self, top_n: int = 20) -> Dict[str, Dict]:
        ...
        """
        if self.xgb_model is None or self.lgb_model is None or self.cat_model is None:
        ...
        cat_importance = self.cat_model.get_feature_importance(top_n=top_n)
