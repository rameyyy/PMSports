...
def load_features_by_year(years: list) -> pl.DataFrame:
    ...
    if not all_features:
        print("  [ERR] No feature files loaded!")
        return None

    # Combine all years - handle schema mismatches by casting consistent types
    # Cast known string columns that should be float to float
    float_cols_to_fix = [
...
def train_lightgbm_model(X_train: pl.DataFrame, y_train: pl.DataFrame,
                        model_dir: Path = None) -> lgb.Booster:
    """
    Train LightGBM classifier for moneyline predictions
    """
    ...
    print("="*80)
    print("TRAINING LIGHTGBM MONEYLINE MODEL (2021-2024)")
    print("="*80 + "\n")
    ...
    # LightGBM parameters - tuned for similar performance to XGBoost
    params = {
        'objective': 'binary',
    ...
    feature_importance = list(zip(X_train.columns, model.feature_importance(importance_type='gain')))
...
def main():
    print("\n")
    print("="*80)
    print("MONEYLINE MODEL - LIGHTGBM TRAIN/TEST PIPELINE")
    print("="*80 + "\n")
    ...
    train_metrics = evaluate_model(y_train, y_pred_binary_train, y_pred_train,
    ...
    test_metrics = evaluate_model(y_test, y_pred_binary_test, y_pred_test,
                                   "Test Set (2025)")
