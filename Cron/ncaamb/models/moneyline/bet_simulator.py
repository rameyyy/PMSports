#!/usr/bin/env python3
"""
Bet Simulator - Test Good Bets Model on 2025 Data
Trains fresh XGBoost + LightGBM on 2021-2024
Tests on 2025 with $10 bet sizes
Analyzes ROI, win rate, and profit by EV ranges
"""
...
def create_good_bets_data(df: pl.DataFrame, y: np.ndarray,
    ...
    Returns: (X_bets, y_bets, game_ids, team_indices)
    """
    ...
    # Calculate EV using vectorized ops (for reference, not for model)
    df = df.with_columns([
    ...
    team_1_data = df.select([
        ...
        pl.lit(1).alias('team_index'),  # Team 1
    ])
    ...
    team_2_data = df.select([
        ...
        pl.lit(2).alias('team_index'),  # Team 2
    ])
    ...
    good_bet_targets = all_bets.select('target').to_numpy().ravel()
    ...
    return X_bets, y_bets, moneyline_odds, good_bet_targets
...
def simulate_bets(good_bet_probs, american_odds, actual_results, bet_size=10):
    ...
    Returns: (profit, win_count, loss_count, roi, win_rate)
    """
    profit = 0
    wins = 0
    losses = 0
    ...
    for prob, odds, result in zip(good_bet_probs, american_odds, actual_results):
        ...
        if result == 1:  # Bet won
            ...
            profit += payout - bet_size  # Profit = payout - stake
            wins += 1
        else:  # Bet lost
            profit -= bet_size
            losses += 1
...
def main():
    print("\n")
    print("="*80)
    print("BET SIMULATOR - GOOD BETS MODEL")
    ...
    X_bets_test, y_bets_test, moneyline_odds, good_bet_targets = create_good_bets_data(
    ...
    print("STEP 8: Analyzing Bet Performance by EV Ranges")
    print("-"*80 + "\n")

    results_df = pl.DataFrame({
        'good_bet_prob': good_bet_probs,
        'american_odds': moneyline_odds,
        'ev': evs,
        'actual_result': good_bet_targets,
    })
    ...
    ev_ranges = [
        ...
        (0, 0.05, 'EV 0% to 5%'),
        (0.05, 0.1, 'EV 5% to 10%'),
        (0.1, 0.15, 'EV 10% to 15%'),
        (0.15, 0.2, 'EV 15% to 20%'),
        (0.2, float('inf'), 'EV > 20%'),
    ]

    print(f"{'EV Range':<25} {'Bets':<8} {'Wins':<8} {'Losses':<8} {'Win %':<10} {'Profit':<12} {'ROI':<10}")
    print("-" * 95)
    ...
    for min_ev, max_ev, label in ev_ranges:
        ...
        range_results = good_bet_targets[mask]
        range_evs = evs[mask]
        ...
        print(f"{label:<25} {num_bets:<8} {wins:<8} {losses:<8} {win_pct:<10.2f}% ${profit:<11.2f} {roi:<10.2f}%")
    ...
    # Overall stats
    print("\n" + "="*80)
    print("OVERALL BETTING SUMMARY")
    print("="*80 + "\n")

    total_profit, total_wins, total_losses, total_roi, total_win_rate = simulate_bets(
        good_bet_probs,
        moneyline_odds,
        good_bet_targets,
        bet_size=10
    ...
    total_wagered = total_bets * 10

    print(f"Total Bets:      {total_bets}")
    print(f"Total Wins:      {total_wins} ({total_win_rate*100:.2f}%)")
    print(f"Total Losses:    {total_losses}")
    print(f"Total Wagered:   ${total_wagered:.2f}")
    print(f"Total Profit:    ${total_profit:.2f}")
    print(f"ROI:             {total_roi:.2f}%")
    print(f"Avg EV:          {np.mean(evs):.4f}")
    print(f"Median EV:       {np.median(evs):.4f}")
    print(f"Std Dev EV:      {np.std(evs):.4f}\n")

    # Find best performing range
    if summary_data:
        best_range = max(summary_data, key=lambda x: x['roi'])
        print(f"Best EV Range:   {best_range['ev_range']}")
        print(f"  ROI: {best_range['roi']:.2f}%")
        print(f"  Profit: ${best_range['profit']:.2f}")
        print(f"  Win Rate: {best_range['win_rate']:.2f}%\n")
    ...
    results_file = Path(__file__).parent / "saved" / "bet_simulation_results.txt"
    with open(results_file, 'w') as f:
        f.write("BET SIMULATION RESULTS - 2025 TEST DATA\n")
        f.write("Models trained on 2021-2024, tested on 2025\n")
        f.write("$10 per bet, decimal odds conversion\n\n")

        f.write("="*95 + "\n")
        f.write("PERFORMANCE BY EV RANGE\n")
        f.write("="*95 + "\n\n")

        f.write(f"{'EV Range':<25} {'Bets':<8} {'Wins':<8} {'Losses':<8} {'Win %':<10} {'Profit':<12} {'ROI':<10}\n")
        f.write("-" * 95 + "\n")

        for data in summary_data:
            f.write(f"{data['ev_range']:<25} {data['num_bets']:<8} {data['wins']:<8} {data['losses']:<8} {data['win_rate']:<10.2f}% ${data['profit']:<11.2f} {data['roi']:<10.2f}%\n")
        ...
        f.write(f"Total Wins:      {total_wins} ({total_win_rate*100:.2f}%)\n")
        f.write(f"Total Losses:    {total_losses}\n")
        f.write(f"Total Wagered:   ${total_wagered:.2f}\n")
        f.write(f"Total Profit:    ${total_profit:.2f}\n")
        f.write(f"ROI:             {total_roi:.2f}%\n\n")

        f.write(f"EV Statistics:\n")
        f.write(f"  Mean:          {np.mean(evs):.4f}\n")
        f.write(f"  Median:        {np.median(evs):.4f}\n")
        f.write(f"  Std Dev:       {np.std(evs):.4f}\n")
        f.write(f"  Min:           {np.min(evs):.4f}\n")
        f.write(f"  Max:           {np.max(evs):.4f}\n\n")

        if summary_data:
            best_range = max(summary_data, key=lambda x: x['roi'])
            f.write(f"Best Performing EV Range: {best_range['ev_range']}\n")
            f.write(f"  ROI: {best_range['roi']:.2f}%\n")
            f.write(f"  Profit: ${best_range['profit']:.2f}\n")
            f.write(f"  Win Rate: {best_range['win_rate']:.2f}%\n")

    print(f"[OK] Results saved to {results_file}\n")

    print("="*80)
    print("[OK] Bet simulation complete!")
    print("="*80 + "\n")
