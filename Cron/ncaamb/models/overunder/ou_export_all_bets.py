#!/usr/bin/env python3
"""
Export all 2025 O/U bets - one row per game with all sportsbooks' lines
Uses pre-trained ensemble (44.1% XGB + 46.6% LGB + 9.3% CatBoost)
Only exports games where ensemble prediction differs from O/U line (potential edge)
"""
...
def main():
    print("\n" + "="*80)
    print("O/U BETS EXPORT - ALL 2025 GAMES")
    print("Using pre-trained ensemble (44.1% XGB + 46.6% LGB + 9.3% CatBoost)")
    print("="*80 + "\n")
    ...
    # Load models
    print("STEP 3: Loading Pre-trained Models")
    print("-"*80 + "\n")
    ...
    print(f"  [+] XGBoost predictions: {xgb_preds.shape}")

    lgb_preds = lgb_booster.predict(X_test)
    print(f"  [+] LightGBM predictions: {lgb_preds.shape}")

    cat_preds = cat_model.predict(X_test)
    print(f"  [+] CatBoost predictions: {cat_preds.shape}")
