#!/usr/bin/env python3
"""
Export clean 2025 good bets - train ensemble + RF on 2021-2024, test on 2025
...
def main():
    print("\n" + "="*80)
    print("CLEAN O/U GOOD BETS EXPORT - TRAIN 2021-2024, TEST 2025")
    ...
    # Train good bets classifier
    print("STEP 5: Building Good Bets Features and Training RF Classifier")
    print("-"*80 + "\n")

    # Get predictions on training data for good bets features
    xgb_train_preds = xgb_model.predict(X_train)
    lgb_train_preds = lgb_model.predict(X_train)
    cat_train_preds = cat_model.predict(X_train)

    # Get O/U lines from training data
    betonline_ou_line_train = train_df['betonline_ou_line'].fillna(0).values
    avg_ou_line_train = train_df.get('avg_ou_line', pd.Series([0]*len(train_df))).fillna(0).values
    ou_line_variance_train = train_df.get('ou_line_variance', pd.Series([0]*len(train_df))).fillna(0).values

    # Calculate confidence scores for each model on training data
    xgb_confidence_over_train = 1.0 / (1.0 + np.exp(-(xgb_train_preds - betonline_ou_line_train) / 3.0))
    lgb_confidence_over_train = 1.0 / (1.0 + np.exp(-(lgb_train_preds - betonline_ou_line_train) / 3.0))
    cat_confidence_over_train = 1.0 / (1.0 + np.exp(-(cat_train_preds - betonline_ou_line_train) / 3.0))

    # Clip to valid range
    xgb_confidence_over_train = np.clip(xgb_confidence_over_train, 0.01, 0.99)
    lgb_confidence_over_train = np.clip(lgb_confidence_over_train, 0.01, 0.99)
    cat_confidence_over_train = np.clip(cat_confidence_over_train, 0.01, 0.99)

    # Ensemble confidence
    ensemble_confidence_over_train = (
        0.441 * xgb_confidence_over_train +
        0.466 * lgb_confidence_over_train +
        0.093 * cat_confidence_over_train
    )

    # Model disagreement
    model_std_dev_train = np.std([xgb_confidence_over_train, lgb_confidence_over_train, cat_confidence_over_train], axis=0)

    # Build feature matrix for Good Bets model (training)
    good_bets_features_train = np.column_stack([
        xgb_confidence_over_train,
        lgb_confidence_over_train,
        cat_confidence_over_train,
        ensemble_confidence_over_train,
        model_std_dev_train,
        betonline_ou_line_train,
        avg_ou_line_train,
        ou_line_variance_train
    ])

    # Create target variable for good bets (1 if actual > betonline line, 0 otherwise)
    good_bets_target_train = (train_df['actual_total'] > train_df['betonline_ou_line']).astype(int).values

    # Train Random Forest classifier
    print("  Training Random Forest good bets classifier...")
    good_bets_model = RandomForestClassifier(
        n_estimators=100,
        max_depth=10,
        random_state=42,
        n_jobs=-1
    )
    good_bets_model.fit(good_bets_features_train, good_bets_target_train)
    print("  [OK] Random Forest trained\n")

    # Make predictions on test data
    print("STEP 6: Making Predictions on 2025 Test Data")
    print("-"*80 + "\n")
    ...
    # Create good bets features for test data
    print("STEP 7: Creating Good Bets Features for Test Data")
    print("-"*80 + "\n")

    betonline_ou_line_test = test_df['betonline_ou_line'].fillna(0).values
    avg_ou_line_test = test_df.get('avg_ou_line', pd.Series([0]*len(test_df))).fillna(0).values
    ou_line_variance_test = test_df.get('ou_line_variance', pd.Series([0]*len(test_df))).fillna(0).values

    # Calculate confidence scores for each model on test data
    xgb_confidence_over_test = 1.0 / (1.0 + np.exp(-(xgb_preds - betonline_ou_line_test) / 3.0))
    lgb_confidence_over_test = 1.0 / (1.0 + np.exp(-(lgb_preds - betonline_ou_line_test) / 3.0))
    cat_confidence_over_test = 1.0 / (1.0 + np.exp(-(cat_preds - betonline_ou_line_test) / 3.0))

    # Clip to valid range
    xgb_confidence_over_test = np.clip(xgb_confidence_over_test, 0.01, 0.99)
    lgb_confidence_over_test = np.clip(lgb_confidence_over_test, 0.01, 0.99)
    cat_confidence_over_test = np.clip(cat_confidence_over_test, 0.01, 0.99)

    # Ensemble confidence
    ensemble_confidence_over_test = (
        0.441 * xgb_confidence_over_test +
        0.466 * lgb_confidence_over_test +
        0.093 * cat_confidence_over_test
    )

    # Model disagreement
    model_std_dev_test = np.std([xgb_confidence_over_test, lgb_confidence_over_test, cat_confidence_over_test], axis=0)

    # Build feature matrix for Good Bets model (test)
    good_bets_features_test = np.column_stack([
        xgb_confidence_over_test,
        lgb_confidence_over_test,
        cat_confidence_over_test,
        ensemble_confidence_over_test,
        model_std_dev_test,
        betonline_ou_line_test,
        avg_ou_line_test,
        ou_line_variance_test
    ])

    # Get good bets probabilities
    good_bets_probs = good_bets_model.predict_proba(good_bets_features_test)[:, 1]
    print(f"  [+] Generated good bets confidence scores\n")

    # Create betting records
    print("STEP 8: Creating Betting Records")
    print("-"*80 + "\n")
