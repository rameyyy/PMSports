#!/usr/bin/env python3
"""
Aggressive parameter optimization focused on lowest Test MAE
Tests up to 300 different parameter combinations
Goal: Find the sweet spot between accuracy and generalization
"""
import polars as pl
import os
import sys
...
def main():
    print("=" * 100)
    print("AGGRESSIVE XGBOOST PARAMETER OPTIMIZATION")
    print("Focus: Lowest Test MAE (actual prediction accuracy)")
    print("=" * 100)
    ...
    # Define parameter search space - more aggressive/complex models
    ...
    print("=" * 100)
    print(f"\n{'Rank':<5} {'Test MAE':<10} {'Train MAE':<10} {'Overfit':<10} {'Test RMSE':<10} {'Parameters':<50}")
    print("-" * 100)

    for i, result in enumerate(results[:30]):
        params = result['params']
        overfit_ratio = result['test_mae'] / result['train_mae'] if result['train_mae'] > 0 else 0
        param_str = f"lr={params['learning_rate']:.2f} d={params['max_depth']} ne={params['n_estimators']} ss={params['subsample']:.1f}"
        print(f"{i+1:<5} {result['test_mae']:<10.2f} {result['train_mae']:<10.2f} {overfit_ratio:<10.2f} {result['test_rmse']:<10.2f} {param_str:<50}")
    ...
    print("=" * 100)
    print(f"\nTest MAE: {best['test_mae']:.2f} points (LOWEST - best for predictions)")
