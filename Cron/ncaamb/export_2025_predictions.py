#!/usr/bin/env python3
"""
Export 2025 test predictions to Excel
Train on 2021-2024, test and export 2025 results
One row per team with model prediction and best available odds
"""
...
def export_2025_predictions_excel(df: pl.DataFrame, predictions_proba: np.ndarray, odds_dict: dict, output_file: str):
    """
    Export 2025 test predictions to Excel with one row per team
    Columns: game_id, date, team, home_away, model_win_prob, best_odds, actual_winner
    """
    rows = []
    ...
    for idx, game in enumerate(df.iter_rows(named=True)):
        ...
        if team_1_odds is not None:
            rows.append({
                ...
                'model_win_prob': round(team_1_win_prob, 4),
        ...
        if team_2_odds is not None:
            rows.append({
                ...
                'model_win_prob': round(team_2_win_prob, 4),
